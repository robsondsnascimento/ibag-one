    ...
    <div className="records-page hierarchy-page">
      ...
      {!isLoading && (
        <>
          ...
          {canManageSupervision && activeNetworks.some((network) => supervisions.filter((supervision) => supervision.ativo && supervision.network.id === network.id).length < 2) && <section className="hierarchy-supervision-form">
            <div><p className="eyebrow">Supervisão</p><h3>Definir supervisores de rede</h3><p>Cada rede pode possuir até dois supervisores ativos.</p></div>
            <form className="event-form" onSubmit={submitSupervision}>
              ...
              <label>Rede<select name="supervisionNetworkId" defaultValue="" required disabled={isSaving || activeNetworks.length === 0}><option value="" disabled>Selecione uma rede com vaga</option>{activeNetworks.filter((network) => supervisions.filter((supervision) => supervision.ativo && supervision.network.id === network.id).length < 2).map((network) => <option value={network.id} key={network.id}>{network.nome} · {network.campus.nome}</option>)}</select></label>
          ...
          <section className="network-card-grid">
            {networks.map((network) => {
              const networkSupervisors = supervisions.filter((item) => item.ativo && item.network.id === network.id)
              ...
              return <article className={`network-card ${network.ativo ? '' : 'network-card--inactive'}`} key={network.id}>
                ...
                <div className="network-card-detail"><span>Supervisores ({networkSupervisors.length}/2)</span>{networkSupervisors.length > 0 ? <div className="network-supervisor-list">{networkSupervisors.map((supervisor) => <div key={supervisor.id}><strong>{supervisor.person.nome}</strong>{canManageSupervision && <button type="button" onClick={() => void onEndSupervision(supervisor.id, supervisor.person.nome)} disabled={isSaving}>Encerrar supervisão</button>}</div>)}</div> : <strong>Não definido</strong>}</div>
