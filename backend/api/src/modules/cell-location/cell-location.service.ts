...
export class CellLocationService {
  ...
  private async authorize(cellId: string, context: OrganizationContext) {
    ...
    const user = await this.prisma.user.findFirst({ where: { id: context.userId, organizationId: context.organizationId, ...userRoleWhere(['SECRETARY', 'ADMIN', 'SUPER_ADMIN']) } }); if (user) return;
