...
export class KidsService {
  ...
  private async assertClassAccess(klass: { serviceAreaId: string; teamId: string }, c: OrganizationContext) { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR'])) return; const full = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, serviceAreaId: klass.serviceAreaId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] } } }); if (full) return; const team = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, teamId: klass.teamId, ativo: true } }); if (!team) throw new ForbiddenException('Sem acesso a esta faixa etária'); }
  private async authorizeOperation(c: OrganizationContext, campusId: string, role: 'CHECK_IN') { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR'])) return; if (await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { personId: c.personId, campusId, role, ativo: true } })) return; if (await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { campusId, ativo: true } } } } })) return; throw new ForbiddenException('Sem função operacional Kids para esta ação'); }
  private async assertSensitiveAccess(c: OrganizationContext) { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR'])) return; const leadership = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { organizationId: c.organizationId, ativo: true } } } } }); if (!leadership) throw new ForbiddenException('Sem acesso operacional completo à IBAG Kids'); }
}
