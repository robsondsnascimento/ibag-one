...
export class KidsService {
  ...
  async createClass(dto: CreateKidsClassDto, c: OrganizationContext) {
    await this.authorize(c, dto.serviceAreaId, dto.campusId);
  ...
  async createChild(dto: CreateKidsChildDto, c: OrganizationContext) {
    await this.assertSensitiveAccess(c, dto.campusId);
  ...
  async careProfile(childId: string, c: OrganizationContext) { const child = await this.person(childId, c); await this.assertSensitiveAccess(c, child.campusId); return this.prisma.kidsCareProfile.findUnique({ where: { childId } }); }
  async childAttendance(childId: string, c: OrganizationContext) { const child = await this.person(childId, c); await this.assertSensitiveAccess(c, child.campusId); return this.prisma.kidsCheckIn.findMany({ where: { childId }, include: { enrollment: { include: { class: true } } }, orderBy: { checkInAt: 'desc' } }); }
  ...
  async upsertCareProfile(childId: string, dto: UpsertKidsCareProfileDto, c: OrganizationContext) { const child = await this.person(childId, c); await this.assertSensitiveAccess(c, child.campusId); return this.prisma.kidsCareProfile.upsert({ where: { childId }, create: { childId, ...dto }, update: dto }); }

  async assignOperationalRole(dto: AssignKidsOperationalRoleDto, c: OrganizationContext) {
    await this.assertSensitiveAccess(c, dto.campusId); await this.person(dto.personId, c);
    if (!await this.prisma.campus.findFirst({ where: { id: dto.campusId, organizationId: c.organizationId, ativo: true } })) throw new NotFoundException('Campus ativo não encontrado');
  ...
  async endOperationalRole(id: string, c: OrganizationContext) { const role = await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { id, ativo: true, organizationId: c.organizationId } }); if (!role) throw new NotFoundException('Função operacional ativa não encontrada'); await this.assertSensitiveAccess(c, role.campusId); return this.prisma.kidsOperationalRoleAssignment.update({ where: { id }, data: { ativo: false, fim: new Date() } }); }
  async transferOperationalRole(id: string, dto: TransferKidsOperationalRoleDto, c: OrganizationContext) { const current = await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { id, ativo: true, organizationId: c.organizationId } }); if (!current) throw new NotFoundException('Função operacional ativa não encontrada'); await this.assertSensitiveAccess(c, current.campusId); await this.person(dto.personId, c); if (await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { personId: dto.personId, campusId: current.campusId, role: current.role, ativo: true } })) throw new BadRequestException('A pessoa já possui esta função ativa no campus'); return this.prisma.$transaction(async tx => { await tx.kidsOperationalRoleAssignment.update({ where: { id }, data: { ativo: false, fim: new Date() } }); return tx.kidsOperationalRoleAssignment.create({ data: { personId: dto.personId, campusId: current.campusId, role: current.role, organizationId: c.organizationId } }); }); }

  async classes(c: OrganizationContext) {
    const pastorCampusId = await this.pastorCampusId(c);
    const areaIds = pastorCampusId ? undefined : await this.accessibleAreaIds(c);
    const classes = await this.prisma.kidsClass.findMany({ where: { organizationId: c.organizationId, ativo: true, ...(pastorCampusId ? { campusId: pastorCampusId } : {}), ...(areaIds ? { serviceAreaId: { in: areaIds } } : {}) }, include: { campus: true, space: true, serviceArea: true, _count: { select: { enrollments: { where: { ativo: true } } } } }, orderBy: { nome: 'asc' } });
  ...
  async enroll(classId: string, dto: EnrollKidDto, c: OrganizationContext) {
    const klass = await this.klass(classId, c); await this.authorize(c, klass.serviceAreaId, klass.campusId); await this.person(dto.childId, c);
  ...
  async authorizePickup(childId: string, dto: AuthorizeKidsPickupDto, c: OrganizationContext) {
    const enrollment = await this.enrollment(childId, c); await this.authorize(c, enrollment.class.serviceAreaId, enrollment.class.campusId); await this.person(dto.responsiblePersonId, c);
  ...
  async openCheckIns(c: OrganizationContext) {
    const pastorCampusId = await this.pastorCampusId(c);
    const areaIds = pastorCampusId ? undefined : await this.accessibleAreaIds(c);
    return this.prisma.kidsCheckIn.findMany({ where: { status: 'CHECKED_IN', enrollment: { class: { organizationId: c.organizationId, ...(pastorCampusId ? { campusId: pastorCampusId } : {}), ...(areaIds ? { serviceAreaId: { in: areaIds } } : {}) } } }, include: { child: true, enrollment: { include: { class: { include: { campus: true, space: true } } } } }, orderBy: { checkInAt: 'asc' } });
  }
  ...
  private async accessibleAreaIds(c: OrganizationContext): Promise<string[] | undefined> { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return undefined; const links = await this.prisma.serviceMembership.findMany({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER','TEAM_LEADER'] } }, select: { serviceAreaId: true } }); if (!links.length) throw new ForbiddenException('Sem acesso à operação IBAG Kids'); return links.map(link => link.serviceAreaId); }
  private async pastorCampusId(c: OrganizationContext): Promise<string | undefined> { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { person: { select: { campusId: true } }, additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['PASTOR_SENIOR'])) return undefined; return hasAnyUserRole(user, ['PASTOR']) ? user?.person?.campusId : undefined; }
  private async authorize(c: OrganizationContext, serviceAreaId: string, campusId: string) { const pastorCampusId = await this.pastorCampusId(c); if (pastorCampusId) { if (pastorCampusId === campusId) return; throw new ForbiddenException('O pastor só pode operar o IBAG Kids do seu campus'); } const ids = await this.accessibleAreaIds(c); if (ids && !ids.includes(serviceAreaId)) throw new ForbiddenException('Sem acesso a esta área Kids'); }
  private async assertClassAccess(klass: { serviceAreaId: string; teamId: string; campusId: string }, c: OrganizationContext) { const pastorCampusId = await this.pastorCampusId(c); if (pastorCampusId) { if (pastorCampusId === klass.campusId) return; throw new ForbiddenException('O pastor só pode acessar turmas do seu campus'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; const full = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, serviceAreaId: klass.serviceAreaId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] } } }); if (full) return; const team = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, teamId: klass.teamId, ativo: true } }); if (!team) throw new ForbiddenException('Sem acesso a esta faixa etária'); }
  private async authorizeOperation(c: OrganizationContext, campusId: string, role: 'CHECK_IN') { const pastorCampusId = await this.pastorCampusId(c); if (pastorCampusId) { if (pastorCampusId === campusId) return; throw new ForbiddenException('O pastor só pode operar o IBAG Kids do seu campus'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; if (await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { personId: c.personId, campusId, role, ativo: true } })) return; if (await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { campusId, ativo: true } } } } })) return; throw new ForbiddenException('Sem função operacional Kids para esta ação'); }
  private async assertSensitiveAccess(c: OrganizationContext, campusId: string) { const pastorCampusId = await this.pastorCampusId(c); if (pastorCampusId) { if (pastorCampusId === campusId) return; throw new ForbiddenException('O pastor só pode acessar dados sensíveis do seu campus'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; const leadership = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { organizationId: c.organizationId, campusId, ativo: true } } } } }); if (!leadership) throw new ForbiddenException('Sem acesso operacional completo à IBAG Kids'); }
}
