...
export class KidsService {
  ...
  async classes(c: OrganizationContext) {
    const pastorCampusIds = await this.pastorCampusIds(c);
    const areaIds = pastorCampusIds ? undefined : await this.accessibleAreaIds(c);
    const classes = await this.prisma.kidsClass.findMany({ where: { organizationId: c.organizationId, ativo: true, ...(pastorCampusIds ? { campusId: { in: pastorCampusIds } } : {}), ...(areaIds ? { serviceAreaId: { in: areaIds } } : {}) }, include: { campus: true, space: true, serviceArea: true, _count: { select: { enrollments: { where: { ativo: true } } } } }, orderBy: { nome: 'asc' } });
  ...
  async openCheckIns(c: OrganizationContext) {
    const pastorCampusIds = await this.pastorCampusIds(c);
    const areaIds = pastorCampusIds ? undefined : await this.accessibleAreaIds(c);
    return this.prisma.kidsCheckIn.findMany({ where: { status: 'CHECKED_IN', enrollment: { class: { organizationId: c.organizationId, ...(pastorCampusIds ? { campusId: { in: pastorCampusIds } } : {}), ...(areaIds ? { serviceAreaId: { in: areaIds } } : {}) } } }, include: { child: true, enrollment: { include: { class: { include: { campus: true, space: true } } } } }, orderBy: { checkInAt: 'asc' } });
  }
  ...
  private async pastorCampusIds(c: OrganizationContext): Promise<string[] | undefined> { const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { person: { select: { campusId: true, campusMemberships: { where: { ativo: true }, select: { campusId: true } } } }, additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['PASTOR_SENIOR'])) return undefined; return hasAnyUserRole(user, ['PASTOR']) ? pastoralCampusIds(user) : undefined; }
  private async authorize(c: OrganizationContext, serviceAreaId: string, campusId: string) { const pastorCampusIds = await this.pastorCampusIds(c); if (pastorCampusIds) { if (pastorCampusIds.includes(campusId)) return; throw new ForbiddenException('O pastor só pode operar o IBAG Kids de seus campi vinculados'); } const ids = await this.accessibleAreaIds(c); if (ids && !ids.includes(serviceAreaId)) throw new ForbiddenException('Sem acesso a esta área Kids'); }
  private async assertClassAccess(klass: { serviceAreaId: string; teamId: string; campusId: string }, c: OrganizationContext) { const pastorCampusIds = await this.pastorCampusIds(c); if (pastorCampusIds) { if (pastorCampusIds.includes(klass.campusId)) return; throw new ForbiddenException('O pastor só pode acessar turmas de seus campi vinculados'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; const full = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, serviceAreaId: klass.serviceAreaId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] } } }); if (full) return; const team = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, teamId: klass.teamId, ativo: true } }); if (!team) throw new ForbiddenException('Sem acesso a esta faixa etária'); }
  private async authorizeOperation(c: OrganizationContext, campusId: string, role: 'CHECK_IN') { const pastorCampusIds = await this.pastorCampusIds(c); if (pastorCampusIds) { if (pastorCampusIds.includes(campusId)) return; throw new ForbiddenException('O pastor só pode operar o IBAG Kids de seus campi vinculados'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; if (await this.prisma.kidsOperationalRoleAssignment.findFirst({ where: { personId: c.personId, campusId, role, ativo: true } })) return; if (await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { campusId, ativo: true } } } } })) return; throw new ForbiddenException('Sem função operacional Kids para esta ação'); }
  private async assertSensitiveAccess(c: OrganizationContext, campusId: string) { const pastorCampusIds = await this.pastorCampusIds(c); if (pastorCampusIds) { if (pastorCampusIds.includes(campusId)) return; throw new ForbiddenException('O pastor só pode acessar dados sensíveis de seus campi vinculados'); } const user = await this.prisma.user.findFirst({ where: { id: c.userId, organizationId: c.organizationId }, include: { additionalRoles: { select: { role: true } } } }); if (hasAnyUserRole(user, ['SECRETARY','ADMIN','SUPER_ADMIN','PASTOR_SENIOR'])) return; const leadership = await this.prisma.serviceMembership.findFirst({ where: { personId: c.personId, ativo: true, role: { in: ['GENERAL_LEADER','CAMPUS_LEADER'] }, serviceArea: { kidsClasses: { some: { organizationId: c.organizationId, campusId, ativo: true } } } } }); if (!leadership) throw new ForbiddenException('Sem acesso operacional completo à IBAG Kids'); }
}
