...
export class PastoralCareService {
  ...
  private async assertAccess(subject: any, context: OrganizationContext) {
    const user = await this.prisma.user.findFirst({ where: { id: context.userId, organizationId: context.organizationId }, include: { person: true } });
    if (!user) throw new ForbiddenException('Usuário sem vínculo organizacional');
    if (['ADMIN', 'SUPER_ADMIN'].includes(user.role)) return;
    if (user.role === 'PASTOR' && user.person.campusId === subject.campusId) return;
